...
正常心电图波形特征（生成正常数据时必须遵守）：
- P波：正向、圆钝，振幅 < 0.25mV，时限 < 0.12s
- PR段：等电位（≈ 基线），无明显偏移
- Q波（正常）：仅存在于侧壁/下壁导联，深度 < R波的1/3，时限 < 0.04s
- R波：正向主波，V1→V6逐渐增高
- S波：V1最深，V1→V6逐渐变浅
- ST段：等电位或轻微偏移（±0.05mV以内），绝对不应有明显抬高或压低
- T波：正向（除aVR和V1），振幅 < 0.5mV，不对称（升支缓降支陡）
- 关键：ST段必须保持接近基线，T波必须正向且清晰可见

程序化分析阈值参考（了解检测标准以优化生成质量）：
...
- 窦性心率 60-100bpm，QRS 80-100ms，QTc < 460ms（Bazett），电轴 -30°~105°
