...
**Python 数据量无需担心**：Python 环境中生成 12 导联 × 成千上万点波形数据完全在能力范围内，不会因为数据点数过多而出错。不要因为预估"数据点太多"而犹豫跳过 Python 计算、改用其他方式、或减少数据密度导致波形失真。直接运行，算完画完。
...
你可以使用 runPythonCode 在浏览器 Python 环境中运行计算（含 numpy + scipy），stdout 结果会立即返回给你。Python 环境已内置以下快捷函数，可直接调用完成绘制，无需通过工具调用再传数据：
...
推荐用法：在 Python 中用 numpy/scipy 计算单个心搏的12导联波形 → 必须用 .tolist() 转为 Python list → 对每个导联调用 ecg_draw_cycle（系统自动重复填满面板）→ 最后用 ecg_draw_rhythm 绘制节律带 → 再通过工具调用 writeInterpretation 和 writeLeadDescriptions 补全解读。这是最省力的 Python 路径，强烈推荐。**严禁将 numpy array 直接传入任何 bridge 函数，必须用 .tolist() 转为纯 Python list（[[t, mV], ...]），否则数据将无法正确绘制。**
...
export function buildOpenAITools() {
    ...
    return [
        ...
        },
        {
            ...
            function: {
                name: 'runPythonCode',
                description: '在浏览器 Pyodide (CPython 3.12 + numpy/scipy) 环境中执行 Python 代码。内置 ecg_init/ecg_draw_lead/ecg_draw_all/ecg_draw_rhythm/ecg_set_header/ecg_get_params 快捷函数，可直接在 Python 中完成画布绘制。变量跨调用保留。',
                parameters: {
                    type: 'object',
                    properties: {
                        code: {
